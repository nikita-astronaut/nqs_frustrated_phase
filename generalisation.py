...
def try_one_dataset(dataset, output, Net, number_runs, train_options, rt = 0.02, lr = 0.0003, gpu = False, sampling = "uniform"):
    ...
    for i in range(number_runs):
        ...
        unique_states = train_set[0].cpu().numpy()
        unique_states = unique_states.dot(2**np.arange(unique_states.shape[1]-1,-1,-1))
        _, unique_states = np.unique(unique_states, return_index = True)
        train_unique = torch.sum(train_set[2][unique_states]).item()
